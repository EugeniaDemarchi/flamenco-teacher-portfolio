...
export function crearPictureResponsive(opciones) {
    ...
    ["avif", "webp"].forEach((ext) => {
        ...
        source.srcset = medidas
            .map((m) => `/ASSETS/IMAGES/${carpeta}/${nombreBase}-${m.nombre}.${ext} ${m.ancho}w`)
            .join(", ");
        if (!soloLarge)
            source.sizes = sizes;
    ...
    img.srcset = medidas
        .map((m) => `/ASSETS/IMAGES/${carpeta}/${nombreBase}-${m.nombre}.jpg ${m.ancho}w`)
        .join(", ");
    if (!soloLarge)
        img.sizes = sizes;
    img.alt = alt;
...
//# sourceMappingURL=crear-picture.js.map